...
df["below"] = (df.loc[:, 1:6] < 0.5).sum(axis=1)
df["above"] = (df.loc[:, 1:6] > 0.5).sum(axis=1)
df["min"] = df.loc[:, 1:6].min(axis=1)
df["max"] = df.loc[:, 1:6].max(axis=1)
df["avg"] = df.loc[:, 1:6].mean(axis=1)
df["sum"] = df.loc[:, 1:6].sum(axis=1)
df["count"] = 6
...
# ShallowCNN_lfcc_I
calcualte_roc_auc(df[0], df[1], "ShallowCNN_lfcc_I")

# ShallowCNN_lfcc_O
calcualte_roc_auc(df[0], df[2], "ShallowCNN_lfcc_O")

# ShallowCNN_mfcc_I
calcualte_roc_auc(df[0], df[3], "ShallowCNN_mfcc_I")

# ShallowCNN_mfcc_O
calcualte_roc_auc(df[0], df[4], "ShallowCNN_mfcc_O")

# TSSD_Wave_I
calcualte_roc_auc(df[0], df[5], "TSSD_Wave_I")

# TSSD_Wave_O
calcualte_roc_auc(df[0], df[6], "TSSD_Wave_O")
